...
from drf_spectacular.utils import OpenApiResponse, extend_schema_view
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=(
            "Liste paginée des entreprises avec leurs informations de base.\n\n"
            "**Pagination:** Utilise cursor pagination pour performance sur 6M+ entreprises.\n\n"
            "**Filtres disponibles:**\n"
            "- `is_active`: Filtrer par statut actif\n"
            "- `code_postal`: Filtrer par code postal\n"
            "- `naf_code`: Filtrer par code NAF\n\n"
            "**Recherche:** Paramètre `search` sur siren, siret, nom, nom_commercial, ville_nom, naf_libelle"
        ),
        tags=["Entreprises"],
    ),
    retrieve=extend_schema(
        ...
        description=(
            ...
            "**Inclut:**\n"
            "- Informations légales (SIREN, SIRET, adresse)\n"
            "- Code NAF et sa sous-catégorie lisible\n"
            "- Liste des dirigeants (si enrichis)\n"
            "- Nombre de fiches ProLocalisation"
        ),
        tags=["Entreprises"],
    ),
)
